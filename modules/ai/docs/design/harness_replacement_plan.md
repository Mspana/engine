...
- **Phase 3 — Parity & policy.** Model dropdown ↔ `model/list`; token usage from `turn/completed`; parse-error reaction hook; approval dialogs; native-tool policy enforcement (§3.2); user-facing sandbox/approval policy cycling (Shift+Tab, per-project persistence); **project-setting toggle: legacy orchestrator ↔ harness driver** for A/B.
