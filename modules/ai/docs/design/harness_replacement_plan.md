...
- **Phase 3 — Parity & policy.** Model dropdown ↔ `model/list`; parse-error reaction hook; approval dialogs; native-tool policy enforcement (§3.2); user-facing sandbox/approval policy cycling (Shift+Tab, per-project persistence); **project-setting toggle: legacy orchestrator ↔ harness driver** for A/B.
  *Parity target (decided 7/27, strengthened same day): bring the UX **and code** in line with modern agentic interfaces — streaming text (plain text for final messages), thinking display, plan/todo panel, tool cards, approvals. Standard convention wins over in-house ideas: redesign existing panel elements where they conflict, and drop legacy dev diagnostics (e.g. per-request token pills) where codex has no natural equivalent rather than reconstructing them.*
